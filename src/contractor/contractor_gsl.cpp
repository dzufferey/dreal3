...
void contractor_gsl::prune(fbbox & b, SMTConfig & config) const {
    ...
    double const T_lb = b.back()[m_time_t].lb();
    double const T_ub = b.back()[m_time_t].ub();
    ...
    if (b.back().max_diam() < config.nra_precision) {
        return;
    }
    bool need_to_run = false;
    for (Enode * e : m_vars_0) {
        if (b.back()[e].diam() > config.nra_precision) {
            need_to_run = true;
            break;
        }
    }
    if (b.back()[m_time_t].diam() > config.nra_precision) {
        need_to_run = true;
    }
    if (!need_to_run) { return; }

    extract_sample_point(b.back(), m_vars_0, m_values);
    extract_sample_point(b.back(), m_pars_0, m_params);
    for (unsigned i = 0; i < m_vars_0.size(); i++) {
        b.back()[m_vars_0[i]] = m_values[i];
    }
    for (unsigned i = 0; i < m_pars_0.size(); i++) {
        b.back()[m_pars_0[i]] = m_params[i];
    }
    ...
    while (t < T_ub) {
        ...
        for (Enode * e : m_vars_t) {
            ...
            auto const & iv_X_t = b.back()[e];
            iv &= iv_X_t;
            if (iv.is_empty()) {
                values_good = false;
                DREAL_LOG_INFO << "GSL Not in Range: " << e
                                << " : " << m_values[i] << " not in " << b.back()[e] << " at t = " << t;
                break;
            }
            i++;
        }
        if (values_good) {
            thread_local static box old_box(b.back());
            old_box = b.back();
            ...
            DREAL_LOG_INFO << "GSL Update: time: " << b.back()[m_time_t] << " ==> " << new_t;
            b.back()[m_time_t] = new_t;
            b.swap();
