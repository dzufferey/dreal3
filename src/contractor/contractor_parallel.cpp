...
void contractor_parallel::prune(fbbox & b, SMTConfig & config) const {
    ...
    vector<fbbox> boxes(m_vec.size(), b);
    ...
    b.front() = boxes[0].front();
    for (unsigned i = 0; i < m_vec.size(); i++) {
        contractor const & c = m_vec[i];
        b.front().intersect(boxes[i].front());
...
void contractor_pseq::prune(fbbox & b, SMTConfig & config) const {
