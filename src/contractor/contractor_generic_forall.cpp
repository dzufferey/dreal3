...
void contractor_generic_forall::handle_disjunction(fbbox & b, std::vector<Enode *> const &vec, bool const p, SMTConfig & config) const {
    ...
    if (!forall_vars.empty()) {
        ...
        box counterexample = find_CE(b.front(), forall_vars, vec, p, config);
        if (counterexample.is_empty()) {
...
void contractor_generic_forall::handle_conjunction(fbbox & b, vector<Enode *> const & vec, bool const p, SMTConfig & config) const {
...
void contractor_generic_forall::handle_atomic(fbbox & b, Enode * body, bool const p, SMTConfig & config) const {
    vector<Enode*> vec;
    vec.push_back(body);
    handle_disjunction(b, vec, p, config);
}
