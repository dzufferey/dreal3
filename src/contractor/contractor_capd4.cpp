...
void intersect_params(box & b, integral_constraint const & ic) {
    vector<Enode*> const & pars_0 = ic.get_pars_0();
...
void contractor_capd_simple::prune(fbbox &, SMTConfig &) const {
    if (m_forward) {
...
void contractor_capd_full::prune(fbbox & b, SMTConfig & config) const {
    ...
    set_params(*m_vectorField, b.back(), ic);

    try {
        if (config.nra_ODE_step > 0) {
            m_solver->setStep(config.nra_ODE_step);
        }
        capd::IVector X_0 = extract_ivector(b.back(), m_vars_0);
        capd::IVector X_t = extract_ivector(b.back(), m_vars_t);
        ibex::Interval const & ibex_T = b.back()[ic.get_time_t()];
        ...
        do {
            // Handle Timeout
            if (m_timeout > 0.0 && b.back().max_diam() > config.nra_precision) {
            ...
            // Invariant Check
            if (m_need_to_check_inv && !check_invariant(s, b.back(), config)) {
                break;
            }
            ...
            if (T.leftBound() <= m_timeMap->getCurrentTime().rightBound()) {
                //                     [     T      ]
                // [     current Time     ]
                bool invariantSatisfied = compute_enclosures(prevTime, T,  b.back(), enclosures, config);
            ...
            if (config.nra_ODE_show_progress) {
                ...
                cout << "\r"
                     ...
                     << "Box Width = " << setw(10) << fixed << setprecision(5) << b.back().max_diam() << "\t";
                cout.flush();
            }
        ...
        if (enclosures.size() > 0 && filter(enclosures, X_t, T)) {
            // SAT
            update_box_with_ivector(b.back(), m_vars_t, X_t);
            // TODO(soonhok): Here we still assume that time_0 = zero.
            b.back()[ic.get_time_t()] = ibex::Interval(T.leftBound(), T.rightBound());
